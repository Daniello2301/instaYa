...
router.post(
  ...
  async function (req, res) {
    try {
      ...
      if (!errors.isEmpty()) {
        res.status(400).json({ messagge: errors.array() });
      }
      ...
      send.status = req.body.status.toUpperCase();
...
router.put("/update/:id", [], async function (req, res) {
  ...
  try {
    ...
    send.dateSend = dateSend;
    send.hourSend = hourSend;
    send.status = status;
    send.large = large;
    send.width = width;
    send.height = height;
    ...
    console.log(error);
    res.status(500).json({msj: error.message}).send("Internal Server error", error.message)
  }
});
